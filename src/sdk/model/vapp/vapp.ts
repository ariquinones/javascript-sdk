...
export class Vapp extends Entity implements EntityWithPerfSamples {
  ...
  async getProductSections(): Promise<Array<ProductSection>> {
    return Iland.getHttp().get(`/vapps/${this.uuid}/product-sections`).then((response) => {
  ...
  async updateProductSections(request: UpdateProductSectionRequest): Promise<Task> {
    return Iland.getHttp().post(`/vapps/${this.uuid}/actions/update-product-section-properties`, request.json)
        .then((response) => {
          const apiTask = response.data as TaskJson;
          return new Task(apiTask);
        });
  ...
  async getAvailableStorageProfiles(): Promise<Array<StorageProfile>> {
    return Iland.getHttp().get(`/vapps/${this.uuid}/available-storage-profiles`).then((response) => {
